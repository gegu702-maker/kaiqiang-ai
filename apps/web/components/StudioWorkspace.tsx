...
export function StudioWorkspace({
  ...
  const fallbackActions = resolveResult?.ok === false ? getFallbackActions(locale) : [];
  const steps = useMemo(
    () =>
          ...
          <div className="rounded-lg border border-white/10 bg-panel/85 p-5 shadow-glow">
            ...
            {notice ? (
              <div className="mt-4 rounded-md border border-amber-300/20 bg-amber-300/10 p-3 text-sm leading-6 text-amber-100">
                <p>{notice}</p>
                {resolveResult?.inputUrl || sourceUrl ? <p className="mt-2 break-words text-xs text-amber-100/75">{resolveResult?.inputUrl || sourceUrl}</p> : null}
  ...
  return [
    {
      label: text.upload,
      icon: Video,
      onClick: () => document.getElementById("avatar-studio")?.scrollIntoView({ behavior: "smooth", block: "start" }),
    },
    {
      label: text.paste,
      icon: FileText,
