...
export function SiteHeader({ authSlot }: { authSlot: ReactNode }) {
  ...
  return (
    <header className={isLanding ? "border-b border-slate-200/70 bg-white/72 text-slate-950 backdrop-blur-xl" : "border-b border-white/10 bg-ink/80 text-slate-300 backdrop-blur"}>
      <nav className={isLanding ? "mx-auto grid h-[88px] max-w-[1536px] grid-cols-[1fr_auto] items-center gap-3 px-6 sm:px-10 lg:grid-cols-[1fr_auto_1fr] lg:px-[58px]" : "mx-auto flex max-w-[1440px] items-center justify-between px-6 py-5 sm:px-10 lg:px-14"}>
        <Link href="/" className="flex min-w-0 items-center gap-3">
          <span className={isLanding ? "relative size-12 overflow-hidden rounded-full border border-slate-200 bg-white" : "relative size-10 overflow-hidden rounded-full border border-white/15 bg-white"}>
            <Image src="/logo.png" alt="KAIQIANG.AI logo" fill sizes="40px" className="object-cover" priority />
          </span>
          <span className={isLanding ? "hidden text-lg font-bold tracking-[0.28em] text-slate-950 min-[420px]:inline sm:text-xl" : "text-sm font-semibold tracking-[0.22em] text-white sm:text-base"}>KAIQIANG.AI</span>
        </Link>

        <div className={isLanding ? "hidden items-center justify-center gap-12 text-lg font-semibold text-slate-950 lg:flex" : "flex items-center gap-1 text-sm text-slate-300 sm:gap-2"}>
        ...
        <div className={isLanding ? "flex min-w-0 items-center justify-end gap-2 text-sm font-semibold text-slate-950 sm:gap-3" : "flex items-center gap-1 text-sm text-slate-300 sm:gap-2"}>
          <div className={isLanding ? "flex rounded-2xl border border-slate-200 bg-white/86 p-1 text-base shadow-sm" : "flex rounded-md border border-white/10 bg-white/[0.03] p-1 text-xs"}>
            <button
              type="button"
